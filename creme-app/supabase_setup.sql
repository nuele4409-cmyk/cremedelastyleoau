...
--  Replaces the get_my_role() call (which may not exist)
--  with a direct inline subquery against user_roles.
--  Also ensures the default settings row exists.
-- ══════════════════════════════════════════════════════

-- 1. Replace the UPDATE policy with a robust inline version
drop policy if exists "Admin update system_settings"  on system_settings;
...
-- 2. Ensure the default row exists (id = 1)
--    system_settings uses serial PK, so the first insert gets id = 1.
--    This is idempotent — does nothing if the row is already there.
...
--  Adds four new boolean columns to system_settings so
--  the admin can show/hide Tickets, Judges, Team, and
--  Lineup tabs independently. Defaults to true (visible).
--  Safe to run on an already-populated database.
-- ══════════════════════════════════════════════════════
